<x-layout>
    <section class="py-[80px]">
        <div class="container mx-auto px-5">
            ...
            <form method="POST" action="{{ route('blogs.store') }}" enctype="multipart/form-data" class="px-8 py-8 bg-gray-200 rounded-md">
                @csrf
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="content" id="content"
                              class="border border-gray-200 py-3 px-3 w-full rounded-[6px] focus:outline-none focus:border-blue-500 transition ease-in-out duration-300 h-[120px]">{{ old('content') }}</textarea>
                    @error('content')
                    <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Create Blog</button>
                </div>
            </form>
